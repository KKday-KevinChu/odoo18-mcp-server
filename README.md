...
| 變數 | 說明 | 預設值 |
|------|------|--------|
| `ODOO_URL` | Odoo 伺服器 URL | `https://odoo.eip.kkday.net/` |
...
1. 登入 Odoo（例如 https://odoo.eip.kkday.net/）
...
claude mcp add odoo-mcp-server -- docker run -i --rm \
  -e ODOO_URL=https://odoo.eip.kkday.net/ \
...
services:
  odoo-mcp:
    ...
    ports:
      - "8000:8000"
    environment:
      - ODOO_URL=https://odoo.eip.kkday.net/
      - ODOO_DATABASE=odoo
