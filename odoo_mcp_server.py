...
Based on twtrubiks/odoo19-mcp-server, adapted for Odoo 18.
...
ODOO_URL = os.getenv("ODOO_URL", "https://sit-odoo.eip.kkday.net/")
ODOO_DATABASE = os.getenv("ODOO_DATABASE", "sit-odoo")
...
def build_record_url(model: str, record_id: int) -> str:
    ...
    Odoo 18 uses the classic web client URL format:
...
mcp = FastMCP("Odoo 18 MCP Server (JSON-RPC)", mask_error_details=True)
...
async def health_check(request):
    ...
    return JSONResponse({"status": "healthy", "service": "odoo18-mcp-server", "version": __version__})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Odoo 18 MCP Server (JSON-RPC)")
    parser.add_argument(
        "--transport",
